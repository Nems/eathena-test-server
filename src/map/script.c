...
} buildin_func[] =
{
    ...
    {
    buildin_sc_start3, "sc_start3", "iiiii*"},
    {
...
/*==========================================
 * Allow up to 4 params fo status change (ufb add on)
 *------------------------------------------
 */
int buildin_sc_start3 (struct script_state *st)
{
    ...
    if (st->end > st->start + 8)    //�w�肵���L���������Ԉُ��ɂ���
        bl = map_id2bl (conv_num
                        (st, &(st->stack->stack_data[st->start + 8])));
    else
        bl = map_id2bl (st->rid);
    if (bl->type == BL_PC
        && ((struct map_session_data *) bl)->state.potionpitcher_flag)
        bl = map_id2bl (((struct map_session_data *) bl)->skilltarget);
    skill_status_change_start (bl, type, val1, val2, val3, val4, tick, 0);
    return 0;
}
