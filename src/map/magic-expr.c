...
//UFB
static int
fun_check_status_change (env_t * env, int args_nr, val_t * result, val_t * args)
{
    character_t *sd = (ETY (0) == BL_PC) ? ARGPC (0) : NULL;
    if (!sd)
        RESULTINT = 0;
    else
		RESULTINT = skill_status_change_active (&sd->bl, ARGINT (1));
    return 0;
}
...
static fun_t functions[] = {
    ...
    {"check_status_change", "ei", 'i', fun_check_status_change},
    {NULL, NULL, '.', NULL}
};
